...
from services.common.Helpers.FestivalHelper import FestivalHelper
...
class FestivalHelperTest(unittest.TestCase):
    ...
    def test_get_no_matches(self):
        self.mock_cursor.fetchall.return_value = [('Thunderdome', date(2021, 10, 30))]
        result = self.helper.get('Unknown_Event_2021.mp3')
        self.assertIsNone(result)

    def test_get_returns_most_specific_match(self):
        self.mock_cursor.fetchall.return_value = [('Rebirth', date(2023, 4, 8)), ('Rebirth Festival', date(2023, 4, 9))]
