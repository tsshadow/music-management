...
def get_db_connection():
    try:
        conn = pymysql.connect(host=os.getenv('DB_HOST', 'localhost'), port=int(os.getenv('DB_PORT', '3306')), user=os.getenv('DB_USER', 'music-management'), password=os.getenv('DB_PASS', ''), db=os.getenv('DB_DB', 'music-management'), cursorclass=pymysql.cursors.DictCursor)
        return conn
    ...
    try:
        ...
        for line in result.strip().split('\n'):
            if line:
                parts = line.split('|')
                if len(parts) >= 2:
                    users.append({'lms_id': parts[0], 'username': parts[1], 'lb_token': parts[2] if len(parts) > 2 else ''})
        return users
    except Exception as e:
