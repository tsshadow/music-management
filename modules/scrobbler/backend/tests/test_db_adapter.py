...
async def add_album(adapter, title: str, *, artist_id: int, release_year: int | None=None, mbid: str | None=None) -> int:
    normalized = normalize_text(title)
    async with adapter.session_factory() as session:
        res = await session.execute(insert(release_groups).values(primary_artist_id=artist_id, title=title, title_normalized=normalized, year=release_year, mbid=mbid))
        await session.commit()
        return int(res.inserted_primary_key[0])

async def add_track(adapter, *, title: str, album_id: int | None, primary_artist_id: int | None, duration_secs: int | None, disc_no: int | None, track_no: int | None, mbid: str | None=None, isrc: str | None=None, acoustid: str | None=None, track_uid: str | None=None) -> int:
    normalized = normalize_text(title)
    async with adapter.session_factory() as session:
        uid = track_uid
        if uid is None:
...
async def link_track_artist(adapter, track_id: int, artists_payload, role: str='primary') -> None:
    async with adapter.session_factory() as session:
...
async def test_adapter_upserts():
    ...
    track_id = await add_track(adapter, title='Song', album_id=album_id, primary_artist_id=artist_id, duration_secs=200, disc_no=None, track_no=1, mbid=None, isrc=None, acoustid=None, track_uid=track_uid)
    ...
    assert await adapter.lookup_album_id(title='Album', artist_id=artist_id, release_year=2024) == album_id
    ...
    listen_id, created = await adapter.insert_listen(user_id=user_id, track_id=track_id, listened_at=listened_at, source='test', source_track_id='1', position_secs=10, duration_secs=200, artist_name_raw='Artist', track_title_raw='Song', album_title_raw='Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
    assert listen_id > 0
    assert created is True
    assert await adapter.count_listens() == 1
    listen_id2, created2 = await adapter.insert_listen(user_id=user_id, track_id=track_id, listened_at=listened_at, source='test', source_track_id='1', position_secs=10, duration_secs=200, artist_name_raw='Artist', track_title_raw='Song', album_title_raw='Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
...
async def test_fetch_listens_supports_period_filters_and_pagination():
    ...
    track1 = await add_track(adapter, title='Track One', album_id=album_id, primary_artist_id=artist_id, duration_secs=210, disc_no=1, track_no=1, mbid=None, isrc=None, acoustid=None, track_uid=None)
    track2 = await add_track(adapter, title='Track Two', album_id=album_id, primary_artist_id=artist_id, duration_secs=200, disc_no=1, track_no=2, mbid=None, isrc=None, acoustid=None, track_uid=None)
    ...
    await adapter.insert_listen(user_id=user_id, track_id=track1, listened_at=listen_day, source='listenbrainz', source_track_id='1', position_secs=30, duration_secs=210, artist_name_raw='Artist', track_title_raw='Track One', album_title_raw='Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
    await adapter.insert_listen(user_id=user_id, track_id=track2, listened_at=previous_day, source='listenbrainz', source_track_id='2', position_secs=None, duration_secs=None, artist_name_raw='Artist', track_title_raw='Track Two', album_title_raw='Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
    rows, total = await adapter.fetch_listens(period='day', value='2025-10-09', limit=100, offset=0)
    assert total == 1
    assert len(rows) == 1
    listen = rows[0]
    ...
    rows_all, total_all = await adapter.fetch_listens(period='all', value=None, limit=1, offset=0)
    assert total_all == 2
...
async def test_fetch_listen_detail_returns_enriched_metadata():
    ...
    track_id = await add_track(adapter, title='Detail Track', album_id=album_id, primary_artist_id=artist_id, duration_secs=250, disc_no=1, track_no=5, mbid='track-mbid', isrc='ISRC12345678', acoustid=None, track_uid=None)
    await link_track_artist(adapter, track_id, [(artist_id, 'primary')])
    await link_track_genre(adapter, track_id, [genre_id])
    listened_at = datetime.now(timezone.utc)
    listen_id, _ = await adapter.insert_listen(user_id=user_id, track_id=track_id, listened_at=listened_at, source='listenbrainz', source_track_id='SRC', position_secs=40, duration_secs=250, artist_name_raw='Detail Artist', track_title_raw='Detail Track', album_title_raw='Detail Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
...
async def test_artist_insights_aggregates_listens():
    ...
    track_main = await add_track(adapter, title='Main Track', album_id=album_id, primary_artist_id=artist_id, duration_secs=200, disc_no=1, track_no=1, mbid=None, isrc=None, acoustid=None, track_uid=None)
    track_guest = await add_track(adapter, title='Guest Track', album_id=album_id, primary_artist_id=artist_id, duration_secs=180, disc_no=1, track_no=2, mbid=None, isrc=None, acoustid=None, track_uid=None)
    ...
    await adapter.insert_listen(user_id=user_id, track_id=track_main, listened_at=base_time, source='listenbrainz', source_track_id='A', position_secs=None, duration_secs=None, artist_name_raw='Insight Artist', track_title_raw='Main Track', album_title_raw='Insight Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
    await adapter.insert_listen(user_id=user_id, track_id=track_guest, listened_at=base_time + timedelta(days=1), source='listenbrainz', source_track_id='B', position_secs=None, duration_secs=None, artist_name_raw='Insight Artist', track_title_raw='Guest Track', album_title_raw='Insight Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
    await adapter.insert_listen(user_id=user_id, track_id=track_guest, listened_at=base_time - timedelta(days=40), source='listenbrainz', source_track_id='C', position_secs=None, duration_secs=None, artist_name_raw='Insight Artist', track_title_raw='Guest Track', album_title_raw='Insight Album', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
...
async def test_album_insights_aggregates_metadata():
    ...
    track_one = await add_track(adapter, title='Song One', album_id=album_id, primary_artist_id=artist_id, duration_secs=210, disc_no=1, track_no=1, mbid=None, isrc=None, acoustid=None, track_uid=None)
    track_two = await add_track(adapter, title='Song Two', album_id=album_id, primary_artist_id=artist_id, duration_secs=205, disc_no=1, track_no=2, mbid=None, isrc=None, acoustid=None, track_uid=None)
    ...
    await adapter.insert_listen(user_id=user_id, track_id=track_one, listened_at=now, source='listenbrainz', source_track_id='1', position_secs=None, duration_secs=None, artist_name_raw='Album Artist', track_title_raw='Song One', album_title_raw='Album Insight', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
    await adapter.insert_listen(user_id=user_id, track_id=track_two, listened_at=now + timedelta(minutes=5), source='listenbrainz', source_track_id='2', position_secs=None, duration_secs=None, artist_name_raw='Album Artist', track_title_raw='Song Two', album_title_raw='Album Insight', raw_payload={}, artist_ids=[artist_id], genre_ids=[genre_id])
