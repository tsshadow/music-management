...
from .deps import get_adapter, get_listenbrainz_export_service, verify_api_key
router = APIRouter(prefix='/export', tags=['listenbrainz'], dependencies=[Depends(verify_api_key)])

@router.post('/listenbrainz')
async def export_listenbrainz(payload: ListenBrainzExportRequest, adapter: DatabaseAdapter=Depends(get_adapter), service: ListenBrainzExportService=Depends(get_listenbrainz_export_service)):
    ...
    result = await service.export_user(user=user, token=token, since=payload.since, listen_type=payload.listen_type, batch_size=payload.batch_size)
