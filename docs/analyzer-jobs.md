...
Large deployments typically combine a generous timeout with `split_paths` to balance throughput and worker utilisation. Extremely large directories can also be split manually (for example, invoking the endpoint for `/mnt/music/A-M` and `/mnt/music/N-Z`) so each job finishes well within the configured timeout.
