...
async def albums(period: str=Query('year', pattern='^(day|month|year|all)$'), value: str | None=Query(None), page: int=Query(1, ge=1), page_size: int=Query(100, ge=1, le=500), service: StatsService=Depends(get_stats_service)):
    """Return listen counts grouped by album for the requested period."""
    try:
        data = await service.albums(period, value, page_size, (page - 1) * page_size)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    return {**data, 'page': page, 'page_size': page_size}
...
async def top_artist_by_genre(year: int=Query(...), service: StatsService=Depends(get_stats_service)):
