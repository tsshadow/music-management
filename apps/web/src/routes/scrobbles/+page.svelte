<script lang="ts">
  ...
  import Home from '$lib/scrobbler/views/Home.svelte';
  ...
  async function loadConfig() {
    loadingConfig = true;
    try {
      const response = await fetch(scrobblerApi.url('/config'));
      if (!response.ok) {
  ...
  async function startImport() {
    ...
    importMessage = '';
    try {
      const response = await fetch(scrobblerApi.url('/import/listenbrainz'), {
        method: 'POST',
  ...
  async function startEnrichment() {
    ...
    enrichmentMessage = '';
    try {
      const response = await fetch(scrobblerApi.url('/enrichment'), {
        method: 'POST',
        headers: buildHeaders(),
        body: JSON.stringify({})
      });
