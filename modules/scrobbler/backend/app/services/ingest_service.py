...
class IngestService:
    ...
    async def ingest_with_status(self, payload: ScrobblePayload) -> tuple[int, bool]:
        ...
        track_uid = make_track_uid(artist=primary_artist_name, title=payload.track.title, album=payload.track.album, duration=payload.track.duration_secs)
        track_id = await self.adapter.lookup_track_id_by_uid(track_uid)
        if track_id is None:
            track_id = await self.adapter.lookup_track_details(title=payload.track.title, artist_id=primary_artist_id, album_id=album_id)
