...
class DummyResponse:
    ...
    def __init__(self, data: dict | None=None, status_code: int=200, json_exc: Exception | None=None):
        self._data = data or {}
...
async def test_fetch_remote_genres_falls_back_to_musicbrainz_tags():
    service = ListenBrainzImportService(SimpleNamespace())
    listen = build_listen(track_metadata={'track_name': 'Example', 'artist_name': 'Artist', 'additional_info': {'recording_mbid': '22222222-2222-2222-2222-222222222222'}})
...
async def test_to_payload_normalizes_soundcloud_album_title():
    service = ListenBrainzImportService(SimpleNamespace())
    listen = build_listen(track_metadata={'track_name': 'Example', 'artist_name': 'Artist', 'release_name': 'Soundcloud (SCANTRAXX) - Week 1'})
    payload = await service._to_payload('tester', listen, SimpleNamespace())
...
async def test_to_payload_normalizes_youtube_album_title_without_parenthesis():
    service = ListenBrainzImportService(SimpleNamespace())
    listen = build_listen(track_metadata={'track_name': 'Example', 'artist_name': 'Artist', 'release_name': 'Youtube Special - Supremacy Warmup'})
    payload = await service._to_payload('tester', listen, SimpleNamespace())
