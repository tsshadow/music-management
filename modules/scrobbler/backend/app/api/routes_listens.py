...
async def list_listens(period: str=Query('day', pattern='^(day|week|month|all)$'), value: str | None=Query(None), page: int=Query(1, ge=1), page_size: int=Query(100, ge=1, le=500), adapter: DatabaseAdapter=Depends(get_adapter)):
    ...
    try:
        rows, total = await adapter.fetch_listens(period=period, value=value, limit=page_size, offset=(page - 1) * page_size)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    return {'items': rows, 'total': total, 'page': page, 'page_size': page_size}

@router.get('/recent', dependencies=[Depends(verify_api_key)])
async def recent_listens(limit: int=Query(10, ge=1, le=100), adapter: DatabaseAdapter=Depends(get_adapter)):
