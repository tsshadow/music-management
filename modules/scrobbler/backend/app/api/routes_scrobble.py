...
from .deps import get_enrichment_queue_service, get_ingest_service, verify_api_key
router = APIRouter(prefix='/scrobble', tags=['scrobble'])

@router.post('', status_code=201, dependencies=[Depends(verify_api_key)])
async def ingest_scrobble(payload: ScrobblePayload, service: IngestService=Depends(get_ingest_service), enrichment_queue: EnrichmentQueueService=Depends(get_enrichment_queue_service)):
