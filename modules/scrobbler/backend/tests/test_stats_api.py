...
async def test_stats_endpoints(client):
    ...
    artists = await client.get('/api/v1/stats/artists', params={'period': 'year', 'value': '2023'})
    ...
    tracks = await client.get('/api/v1/stats/tracks', params={'period': 'all'})
    ...
    genres = await client.get('/api/v1/stats/genres', params={'period': 'year', 'value': '2023'})
    assert genres.status_code == 200
    genre_names = [row['genre'] for row in genres.json()['items']]
    assert 'Chill' in genre_names
    month_genres = await client.get('/api/v1/stats/genres', params={'period': 'month', 'value': '2023-11'})
    ...
    day_artists = await client.get('/api/v1/stats/artists', params={'period': 'day', 'value': '2023-05-20'})
