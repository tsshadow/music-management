...
class FestivalHelper:
    ...
    def get(self, input_string: str) -> Optional[dict]:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                matches = [(festival, date) for festival, date in rows if festival and festival.lower() in input_lower]
                if not matches:
                    logging.debug('[FestivalHelper] No matching festival found.')
                    return None
                matches.sort(key=lambda x: len(x[0]), reverse=True)
                best_match = matches[0]
                return {'festival': best_match[0], 'year': year, 'date': best_match[1].isoformat()}
    ...
    def _extract_year(self, text: str) -> Optional[int]:
        ...
        """
        match = re.search('\\b(20\\d{2})\\b', text)
