<script lang="ts">
  /** Interactive form for queueing importer jobs with optional repeat and account parameters. */
  import { createEventDispatcher, onDestroy } from 'svelte';
  import { getJobsContext } from './jobs-context';
  ...
  let accountOptions: Record<AccountKind, string[]> = { soundcloud: [], youtube: [] };
  let selectedAccounts: Record<AccountKind, string> = { soundcloud: '', youtube: '' };
  ...
  type AccountKind = (typeof accountStepMap)[keyof typeof accountStepMap];
  ...
  $: accountType = accountStepMap[selected as keyof typeof accountStepMap] ?? '';
  ...
  $: if (accountType && selectedAccount && !availableAccounts.includes(selectedAccount)) {
    const kind = accountType as AccountKind;
    selectedAccount = '';
    selectedAccounts = { ...selectedAccounts, [kind]: '' };
  }
  ...
  async function startSelected() {
    ...
    if (breakOnExisting && breakOnExistingSteps.includes(selected)) options.breakOnExisting = true;
...
<div class="space-y-4 rounded border border-green-700/40 bg-black/60 p-4">
  <select class="rounded border border-green-700 bg-gray-900 p-2 text-green-400" bind:value={selected}>
    {#each $steps as step}
      <option value={step}>{step}</option>
    {/each}
  </select>
  <label class="ml-2 inline-flex items-center gap-2 text-sm">
    <input class="h-4 w-4 rounded border-green-700 bg-gray-900 text-green-500" type="checkbox" bind:checked={repeat} />
    Repeat
  </label>
  <input
  ...
  {#if redownloadSteps.includes(selected)}
    <label class="ml-2 inline-flex items-center gap-2 text-sm">
      <input class="h-4 w-4 rounded border-green-700 bg-gray-900 text-green-500" type="checkbox" bind:checked={redownload} />
  ...
  {/if}
  <button class="rounded bg-green-600 px-4 py-2 font-bold text-black shadow hover:bg-green-500" on:click={startSelected}>
    Start
  </button>
</div>
