...
class DiscogsLookup:
    ...
    def is_known_artist(self, name: str) -> bool:
        if not self.token:
            return False
        try:
            response = requests.get(self.API_URL, params={'q': name, 'type': 'artist', 'token': self.token}, timeout=5)
...
class LastfmLookup:
    ...
    def is_known_artist(self, name: str) -> bool:
        if not self.api_key:
            return False
        try:
            response = requests.get(self.API_URL, params={'method': 'artist.getinfo', 'artist': name, 'api_key': self.api_key, 'format': 'json'}, timeout=5)
            if response.status_code != 200:
                return False
            data = response.json()
...
class SpotifyLookup:
    ...
    def is_known_artist(self, name: str) -> bool:
        ...
        if not token:
            return False
        try:
            response = requests.get(self.SEARCH_URL, params={'q': name, 'type': 'artist', 'limit': 1}, headers={'Authorization': f'Bearer {token}'}, timeout=5)
            if response.status_code != 200:
                return False
            data = response.json()
...
class ExternalArtistLookup:
    ...
    def __init__(self):
        self.services = [DiscogsLookup(), MusicBrainzLookup(), LastfmLookup(), SpotifyLookup()]
