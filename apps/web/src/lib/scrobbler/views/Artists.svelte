<script lang="ts">
  /** Artist leaderboard with hype trend insight modal. */
  import { onMount } from 'svelte';
  import { scrobblerApi } from '$lib/api';
  import DetailPanel from '../components/DetailPanel.svelte';
  import HypeGraph from '../components/HypeGraph.svelte';
  import StatsLeaderboard from '../components/StatsLeaderboard.svelte';
  import { type LeaderboardRow, type HypePoint } from '../types';

  export let title = 'Most played artists';
  export let description = 'Discover which artists dominate your soundtrack for each period.';
  export let endpoint = scrobblerApi.url('/stats/artists');
  ...
  async function openInsight(row: ArtistRow) {
    ...
    insight = null;
    try {
      const response = await fetch(scrobblerApi.url(`/library/artists/${row.artist_id}/insights`));
      if (!response.ok) {
        throw new Error('Failed to load artist insights');
      }
  ...
  function onSelect(event: CustomEvent<LeaderboardRow>) {
    ...
    openInsight(event.detail as ArtistRow);
  }
...
{#if showInsights}
  ...
  >
    {#if insight}
      ...
      <section class="detail-section">
        <h4>Listening hype</h4>
        <HypeGraph points={hypePoints} />
        {#if hypeHighlights.length > 0}
          <p class="muted">
            Peaks during {hypeHighlights
              .map((highlight) => `${highlight.period} (${highlight.count.toLocaleString()}×)`)
              .join(', ')}.
          </p>
        {:else}
          <p class="muted">No listening history for this artist yet.</p>
        {/if}
      </section>
      <section class="detail-section">
        <h4>Top genres</h4>
