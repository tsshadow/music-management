...
def get_db_connection():
    ...
    if host == 'db':
        host = 'localhost'
    try:
        conn = pymysql.connect(host=host, port=int(os.getenv('DB_PORT', '3306')), user=os.getenv('DB_USER', 'music-management'), password=os.getenv('DB_PASS', ''), database=os.getenv('DB_DB', 'music-management'), cursorclass=pymysql.cursors.DictCursor, connect_timeout=5)
        return conn
    except Exception as e:
        if host == 'localhost':
            try:
                conn = pymysql.connect(host='192.168.1.27', port=int(os.getenv('DB_PORT', '3306')), user=os.getenv('DB_USER', 'music-management'), password=os.getenv('DB_PASS', ''), database=os.getenv('DB_DB', 'music-management'), cursorclass=pymysql.cursors.DictCursor, connect_timeout=2)
                return conn
            except:
                pass
