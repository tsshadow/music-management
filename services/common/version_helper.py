...
def get_version() -> str:
    ...
    paths = ['VERSION', '../VERSION', '../../VERSION', '/app/VERSION']
    for path in paths:
...
def get_release_notes(service_name: Optional[str]=None) -> str:
    ...
    paths.extend(['RELEASE_NOTES.md', '../RELEASE_NOTES.md', '../../RELEASE_NOTES.md', '/app/RELEASE_NOTES.md'])
    for path in paths:
        if os.path.exists(path):
            with open(path, 'r', encoding='utf-8') as f:
                content = f.read()
...
def get_changelog() -> str:
    ...
    paths = ['CHANGELOG.md', '../CHANGELOG.md', '../../CHANGELOG.md', '/app/CHANGELOG.md']
    for path in paths:
        if os.path.exists(path):
            with open(path, 'r', encoding='utf-8') as f:
                content = f.read()
