...
router = APIRouter(prefix='/enrichment', tags=['enrichment'], dependencies=[Depends(verify_api_key)])

@router.post('', status_code=status.HTTP_202_ACCEPTED, response_model=EnrichmentResponse)
async def queue_enrichment_job(payload: EnrichmentRequest | None=None, service: EnrichmentQueueService=Depends(get_enrichment_queue_service)) -> EnrichmentResponse:
