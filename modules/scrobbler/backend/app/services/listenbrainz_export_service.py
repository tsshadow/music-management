...
class ListenBrainzExportService:
    ...
    def __init__(self, adapter: DatabaseAdapter, *, base_url: str='https://api.listenbrainz.org/1', client_factory: Callable[..., httpx.AsyncClient] | None=None) -> None:
        self.adapter = adapter
        self.base_url = base_url.rstrip('/')
        self._client_factory = client_factory or (lambda **kwargs: httpx.AsyncClient(**kwargs))

    async def export_user(self, *, user: str, token: str, since: datetime | None=None, listen_type: str='import', batch_size: int=100) -> dict[str, int]:
        ...
        async with self._client_factory(base_url=self.base_url, headers=headers, timeout=30.0) as client:
            while True:
                rows = await self.adapter.fetch_listens_for_export(user=user, since=since, limit=batch_size, offset=offset)
                if not rows:
                    break
                payload = []
                for row in rows:
                ...
                if payload:
                    response = await client.post('/submit-listens', json={'listen_type': listen_type, 'payload': payload})
    ...
    def _to_track_metadata(row: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not artist_name:
            return None
        metadata: dict[str, Any] = {'track_name': title, 'artist_name': artist_name}
