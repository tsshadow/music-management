<script lang="ts">
  /** Simple SVG sparkline visualising listening hype over time. */
  import { type HypePoint } from '../types';
...
<div class="graph" role="img" aria-label="Listening history for selected artist">
  {#if count === 0}
    <p class="empty">No listening data yet</p>
  {:else}
