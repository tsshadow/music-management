<script lang="ts">
  /** Landing dashboard for recent listens and top-level insights. */
  import { onMount } from 'svelte';
  import { scrobblerApi } from '$lib/api';
  import DetailPanel from '../components/DetailPanel.svelte';
  import KpiCard from '../components/KpiCard.svelte';
  import RecentListensTable from '../components/RecentListensTable.svelte';
  import { type ListenArtist, type ListenRow } from '../types';
  ...
  async function loadSummary() {
    const response = await fetch(scrobblerApi.url('/listens/count'));
    if (response.ok) {
  ...
  async function loadListens() {
    ...
    try {
      ...
      const response = await fetch(`${scrobblerApi.url('/listens')}?${params.toString()}`);
      if (!response.ok) {
        throw new Error('Failed to load listens');
      }
  ...
  function shiftPeriod(direction: number) {
    ...
    try {
      ...
      error = err instanceof Error ? err.message : 'Failed to adjust period';
    }
  }
  ...
  async function showListenDetail(event: CustomEvent<ListenRow>) {
    ...
    panelLoading = true;
    try {
      const response = await fetch(scrobblerApi.url(`/listens/${listen.id}`));
      if (!response.ok) {
        throw new Error('Failed to load listen details');
      }
  ...
  async function showArtistDetail(event: CustomEvent<{ listen: ListenRow; artist: ListenArtist }>) {
    ...
    if (!artist.id) {
      ...
      resetPanel();
      panelError = 'No additional details available for this artist.';
      return;
    }
    panelType = 'artist';
    panelTitle = artist.name;
    ...
    panelLoading = true;
    try {
      const response = await fetch(scrobblerApi.url(`/library/artists/${artist.id}/insights`));
      if (!response.ok) {
  ...
  async function showAlbumDetail(event: CustomEvent<{ listen: ListenRow }>) {
    ...
    if (!listen.album_id) {
      ...
      resetPanel();
      panelError = 'No additional details available for this album.';
      return;
    }
    panelType = 'album';
    panelTitle = listen.album_title ?? 'Album';
    ...
    panelLoading = true;
    try {
      const response = await fetch(scrobblerApi.url(`/library/albums/${listen.album_id}/insights`));
      if (!response.ok) {
  ...
  {:else}
    <div class="table-wrapper">
      ...
      <footer class="pagination">
        {#if total === 0}
          <span>No listens found for this period.</span>
        {:else}
        ...
        {#each albumInsight.tracks as track}
          <li>
            {#if track.disc_no != null}
              <span class="muted">Disc {track.disc_no} · </span>
            {/if}
            {track.track}
            <span class="muted">
