...
class ListenBrainzImportService:
    ...
    def __init__(self, ingest_service: IngestService, *, base_url: str='https://api.listenbrainz.org/1', musicbrainz_base_url: str='https://musicbrainz.org/ws/2', musicbrainz_user_agent: str='scrobbler/1.0 (https://github.com/)', client_factory: Callable[..., httpx.AsyncClient] | None=None) -> None:
        """Store dependencies and prepare the async client factory."""
        self.ingest_service = ingest_service
        self.base_url = base_url.rstrip('/')
        self.musicbrainz_base_url = musicbrainz_base_url.rstrip('/') if musicbrainz_base_url else ''
    ...
    async def import_user(self, *, user: str, token: str | None=None, since: datetime | None=None, page_size: int=100, max_pages: int | None=None) -> dict[str, int]:
        ...
        async with self._client_factory(base_url=self.base_url, headers=headers, timeout=30.0) as client:
            while True:
    ...
    async def _to_payload(self, user: str, listen: dict[str, Any], client: httpx.AsyncClient) -> ScrobblePayload | None:
        ...
        track_no = self._coerce_int(additional.get('tracknumber') or additional.get('track_number'))
        disc_no = self._coerce_int(additional.get('discnumber') or additional.get('disc_number'))
    ...
    def _split_artist_credit(credit: str) -> list[str]:
        ...
        if not credit:
            return []
        parts = [credit]
        for pattern in ('\\s+(?:feat\\.?|ft\\.?|featuring)\\s+', '\\s+(?:vs\\.?|x|\\+)\\s+', '\\s+with\\s+', '\\s+and\\s+', ',', '&'):
            new_parts: list[str] = []
    ...
    async def _fetch_musicbrainz_tags(self, recording_mbid: str) -> list[str]:
        ...
        if not self.musicbrainz_base_url:
            return []
        try:
            async with self._client_factory(base_url=self.musicbrainz_base_url, headers={'User-Agent': self.musicbrainz_user_agent, 'Accept': 'application/json'}, timeout=30.0) as client:
                response = await client.get(f'/recording/{recording_mbid}', params={'inc': 'tags', 'fmt': 'json'})
        except httpx.HTTPError:
            return []
        if response.status_code >= 400:
            return []
    ...
    def _get_recording_mbid(listen: dict[str, Any]) -> str | None:
        ...
        metadata = listen.get('track_metadata') or {}
        additional = metadata.get('additional_info') or {}
        for value in (additional.get('recording_mbid'), metadata.get('recording_mbid'), listen.get('recording_mbid')):
