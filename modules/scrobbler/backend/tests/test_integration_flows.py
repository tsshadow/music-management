...
async def _analyze_track(repository: AnalyzerRepository, *, title: str, artist: str | None=None, genre: str | None=None, duration: int | None=None) -> dict[str, int | str | None]:
    """Helper that stores media library data similar to the analyzer."""
    artist_id: int | None = None
    if artist:
        normalized_artist = normalize_text(artist)
        artist_id = await repository.upsert_artist(display_name=artist, name_normalized=normalized_artist, sort_name=normalized_artist, mbid=None)
    track_uid = make_track_uid(artist, title, None, duration)
    track_id = await repository.upsert_track(title=title, title_normalized=normalize_text(title), album_id=None, primary_artist_id=artist_id, duration=duration, mbid=None, isrc=None, acoustid=None, track_uid=track_uid)
    if artist_id is not None:
        await repository.link_track_artists(track_id, [(artist_id, 'primary')])
    genre_id: int | None = None
    if genre:
        genre_id = await repository.upsert_genre(name=genre, name_normalized=normalize_text(genre))
        await repository.link_track_genres(track_id, [genre_id])
    return {'track_id': track_id, 'artist_id': artist_id, 'genre_id': genre_id, 'track_uid': track_uid}
...
async def test_scrobble_links_to_existing_track(isolated_database):
    ...
    async with adapter.session_factory() as session:
        ...
        linked_artists = (await session.execute(select(listen_artists).where(listen_artists.c.listen_id == listen_id))).fetchall()
...
async def test_reingest_links_existing_listen_to_library(isolated_database):
    ...
    async with adapter.session_factory() as session:
        initial_row = (await session.execute(select(listens.c.track_id).where(listens.c.id == listen_id))).scalar_one()
        assert initial_row is None
    analyzed = await _analyze_track(repository, title='Echo Trail', artist='Signal Forms', duration=200)
    second_id, created_again = await ingest.ingest_with_status(payload)
    assert created_again is False
    assert second_id == listen_id
    async with adapter.session_factory() as session:
        updated_row = (await session.execute(select(listens.c.track_id, listens.c.artist_name_raw).where(listens.c.id == listen_id))).mappings().one()
...
async def test_ingest_selects_consistent_track_when_duplicates_exist(isolated_database):
    ...
    assert created is True
    async with adapter.session_factory() as session:
        stored_track_id = (await session.execute(select(listens.c.track_id).where(listens.c.id == listen_id))).scalar_one()
...
async def test_deduplication_service_merges_duplicate_rows(isolated_database):
    ...
    adapter, repository, _ = isolated_database
    analyzed = await _analyze_track(repository, title='Parallel Drift', artist='Orbit Nine', genre='Downtempo', duration=180)
    user_id = await adapter.upsert_user('listener')
    async with adapter.session_factory() as session:
        raw_one = await session.execute(insert(listens_raw).values(user_id=user_id, source='listenbrainz', source_track_id='abc', payload_json='{}', listened_at=datetime(2024, 6, 1, 8, 0, tzinfo=timezone.utc)))
        raw_one_id = int(raw_one.inserted_primary_key[0])
        raw_two = await session.execute(insert(listens_raw).values(user_id=user_id, source='listenbrainz', source_track_id='def', payload_json='{}', listened_at=datetime(2024, 6, 1, 8, 0, tzinfo=timezone.utc)))
        raw_two_id = int(raw_two.inserted_primary_key[0])
        first_listen = await session.execute(insert(listens).values(raw_id=raw_one_id, user_id=user_id, track_id=None, listened_at=datetime(2024, 6, 1, 8, 0, tzinfo=timezone.utc), source='listenbrainz', source_track_id='abc', artist_name_raw='', track_title_raw='', album_title_raw=''))
        _first_listen_id = int(first_listen.inserted_primary_key[0])
        second_listen = await session.execute(insert(listens).values(raw_id=raw_two_id, user_id=user_id, track_id=analyzed['track_id'], listened_at=datetime(2024, 6, 1, 8, 0, tzinfo=timezone.utc), source='listenbrainz', source_track_id='def', artist_name_raw='Orbit Nine', track_title_raw='Parallel Drift', album_title_raw=''))
        second_listen_id = int(second_listen.inserted_primary_key[0])
        await session.execute(insert(listen_artists).values(listen_id=second_listen_id, artist_id=analyzed['artist_id']))
        await session.execute(insert(listen_genres).values(listen_id=second_listen_id, genre_id=analyzed['genre_id']))
        await session.commit()
    service = DeduplicationService(adapter)
    removed = await service.deduplicate()
    assert removed == 1
    async with adapter.session_factory() as session:
        remaining = (await session.execute(select(listens).where(listens.c.user_id == user_id))).mappings().all()
        ...
        linked_artists = (await session.execute(select(listen_artists).where(listen_artists.c.listen_id == row['id']))).fetchall()
        assert len(linked_artists) == 1
        linked_genres = (await session.execute(select(listen_genres).where(listen_genres.c.listen_id == row['id']))).fetchall()
        assert len(linked_genres) == 1