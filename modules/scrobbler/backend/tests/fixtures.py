...
async def seed_dataset(client: AsyncClient) -> None:
    ...
    for payload in payloads:
        ...
        if primary_artist_name:
            ...
            artist_id = await repo.upsert_artist(display_name=primary_artist_name, name_normalized=normalized, sort_name=normalized, mbid=None)
        album_id = None
        if track.get('album') and artist_id is not None:
            album_id = await repo.upsert_album(title=track['album'], title_normalized=normalize_text(track['album']), artist_id=artist_id, year=track.get('album_year'), mbid=None)
        track_uid = make_track_uid(artist=primary_artist_name, title=track['title'], album=track.get('album'), duration=track.get('duration_secs'))
        track_id = await repo.upsert_track(title=track['title'], title_normalized=normalize_text(track['title']), album_id=album_id, primary_artist_id=artist_id, duration=track.get('duration_secs'), mbid=None, isrc=None, acoustid=None, track_uid=track_uid)
