...
async def test_listenbrainz_export_endpoint(client):
    ...
    await adapter.update_config({'listenbrainz_user': 'exporter', 'listenbrainz_token': 'secret'})
    ...
    try:
        ...
        assert dummy_service.calls == [{'user': 'exporter', 'token': 'secret', 'since': None, 'listen_type': 'import', 'batch_size': 100}]
    finally:
