...
class GenrePredictor:
    ...
    def predict(self, file_path, save=True):
        ...
        if not results:
            return None
        audio_features = ['tempo', 'mean_spectral_centroid', 'mean_spectral_rolloff', 'mean_rms', 'mean_zcr', 'mfcc_1', 'mfcc_2', 'mfcc_3', 'mfcc_4', 'mfcc_5', 'mean_chroma']
    ...
    def save_prediction(self, file_path, genre, confidence):
        ...
        try:
            with self.analyzer.engine.begin() as conn:
                conn.execute(text(query), {'tid': track_id, 'ver': 'v1.0-rf', 'genre': genre, 'conf': confidence})
