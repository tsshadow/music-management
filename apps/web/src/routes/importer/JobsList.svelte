<script lang="ts">
  /** Dashboard of in-flight importer jobs with live log viewer. */
  import { onDestroy, onMount } from 'svelte';
  import { getJobsContext } from './jobs-context';

  const { jobs, stop, get } = getJobsContext();
  let selected: Awaited<ReturnType<typeof get>> | null = null;
  ...
  function duration(job: { started?: string; ended?: string }) {
