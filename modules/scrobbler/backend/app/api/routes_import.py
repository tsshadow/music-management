...
from .deps import get_adapter, get_enrichment_queue_service, get_listenbrainz_service, verify_api_key
router = APIRouter(prefix='/import', tags=['listenbrainz'], dependencies=[Depends(verify_api_key)])

@router.post('/listenbrainz')
async def import_listenbrainz(payload: ListenBrainzImportRequest, adapter: DatabaseAdapter=Depends(get_adapter), service: ListenBrainzImportService=Depends(get_listenbrainz_service), enrichment_queue: EnrichmentQueueService=Depends(get_enrichment_queue_service)):
    ...
    result = await service.import_user(user=user, token=token, since=payload.since, page_size=payload.page_size, max_pages=payload.max_pages)
